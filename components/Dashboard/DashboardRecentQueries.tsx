...
import { ArrowRight, Terminal, CheckCircle2, AlertCircle, Search, Copy, Check, Trash2, X, Play, History } from 'lucide-react';
      ...
      <div>
        ...
        <div className="flex items-center justify-between mb-2">
          <div className="flex items-center gap-2 flex-wrap">
            <Terminal className="w-3.5 h-3.5 text-monokai-cyan" />
            <h3 className="text-xs font-semibold text-monokai-fg tracking-wide">最近的查询</h3>
            ...
            {latencyFilter !== 'all' && onLatencyFilterChange && (
              <span className="inline-flex items-center gap-1 text-2xs font-mono px-1.5 py-0.5 rounded bg-monokai-surface text-monokai-yellow border border-monokai-yellow/40">
                <span>{latencyFilter === 'fast' ? '<50ms' : latencyFilter === 'normal' ? '50-200ms' : '>200ms 慢查询'}</span>
                <button
                  type="button"
          ...
          <div className="flex items-center gap-2">
            {onClearQueries && queries.length > 0 && (
              showClearConfirm ? (
                <div className="flex items-center gap-1.5 bg-monokai-pink/15 border border-monokai-pink/40 rounded-md px-2 py-0.5 text-2xs shadow-2xs">
                  <span className="text-monokai-pink font-semibold">清空?</span>
                  <button
                    type="button"
                    onClick={() => {
                      onClearQueries();
                    ...
                    className="text-monokai-pink font-bold hover:underline cursor-pointer px-1"
                  >
                    是
                  </button>
                  <button
                    ...
                    className="text-monokai-comment hover:text-monokai-fg cursor-pointer px-0.5"
                  >
                    否
                  </button>
                </div>
                ...
                <button
                  ...
                  title="清空查询历史"
                  className="p-1 rounded hover:bg-monokai-pink/20 text-monokai-comment hover:text-monokai-pink transition-colors cursor-pointer focus-visible:outline-none focus-visible:ring-1 focus-visible:ring-monokai-pink"
                >
            ...
            <button
              ...
              className="flex items-center gap-1 text-meta text-monokai-comment hover:text-monokai-cyan transition-colors cursor-pointer rounded px-1"
        ...
        <div className="flex items-center gap-2 mb-2">
          ...
          {onStatusFilterChange && (
            <div className="flex items-center bg-monokai-elevated rounded-lg p-0.5 border border-monokai-border text-2xs shrink-0">
              <button
                type="button"
                onClick={() => onStatusFilterChange('all')}
                className={`px-2 py-0.5 rounded cursor-pointer transition-colors ${statusFilter === 'all' ? 'bg-monokai-surface border border-monokai-border text-monokai-fg font-bold' : 'text-monokai-comment hover:text-monokai-fg'}`}
              >
                全部
              </button>
              <button
                ...
                className={`px-2 py-0.5 rounded cursor-pointer transition-colors ${statusFilter === 'success' ? 'bg-monokai-accent/20 text-monokai-accent border border-monokai-accent/30 font-bold' : 'text-monokai-comment hover:text-monokai-fg'}`}
              >
                成功
              </button>
              <button
                ...
                className={`px-2 py-0.5 rounded cursor-pointer transition-colors ${statusFilter === 'error' ? 'bg-monokai-pink/20 text-monokai-pink border border-monokai-pink/30 font-bold' : 'text-monokai-comment hover:text-monokai-fg'}`}
              >
                异常
              </button>
            </div>
            ...
            {displayedQueries.map(q => (
              ...
              >
                ...
                <div className="col-span-6 flex items-center gap-1.5 min-w-0 pr-2">
                  <span
                    className={`w-1.5 h-1.5 rounded-full shrink-0 ${q.status === 'error' ? 'bg-monokai-pink shadow-[0_0_6px_#f92672]' : 'bg-monokai-accent'}`}
                  />
