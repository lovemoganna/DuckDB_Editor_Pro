  ...
  return (
    <div
      className={`group flex items-center gap-2 px-3 py-2 rounded-lg cursor-pointer transition-all text-xs ${
        isActive
          ? 'bg-monokai-surface border border-monokai-border-strong text-monokai-fg shadow-xs'
          : 'hover:bg-monokai-bg/60 border border-transparent'
      }`}
      onClick={!editing ? onSelect : undefined}
    >
      <MessageSquare className={`w-3.5 h-3.5 flex-shrink-0 ${isActive ? 'text-monokai-accent' : 'text-monokai-fg-muted/60'}`} />
      ...
      <div className="flex-1 min-w-0">
        {editing ? (
          <input
            ...
            className="w-full bg-monokai-bg border border-monokai-accent rounded px-1.5 py-0.5 text-xs text-monokai-fg outline-none"
    ...
    return (
      <div className="flex gap-2 justify-end">
        <div className="max-w-[85%]">
          ...
          <div className="bg-monokai-surface border border-monokai-border rounded-xl rounded-tr-sm px-3 py-2.5 shadow-xs">
            <p className="text-xs text-monokai-fg leading-relaxed">
              <span className="font-semibold text-monokai-accent">{request?.operation && OPERATION_CONFIG[request.operation]?.label}</span>
        ...
        <div className="w-6 h-6 rounded-full bg-monokai-surface border border-monokai-border flex items-center justify-center flex-shrink-0 mt-5">
          <span className="text-[9px] text-monokai-fg font-bold">U</span>
        </div>
      </div>
    );
  }
    ...
    <div className="flex gap-2 justify-start">
      ...
      <div className="max-w-[85%]">
        <div className="flex items-center gap-1.5 mb-1">
          ...
          {rawSummary && <span className="text-[10px] text-monokai-fg-muted">{rawSummary}</span>}
        </div>

        {isPending ? (
          <div className="bg-monokai-surface border border-monokai-border rounded-xl rounded-tl-sm px-3 py-3">
            <div className="flex items-center gap-2">
              <RefreshCw className="w-3.5 h-3.5 text-monokai-accent animate-spin" />
              <span className="text-xs text-monokai-fg-muted">AI 思考中...</span>
            </div>
          </div>
        ) : error ? (
          <div className="bg-monokai-surface border border-rose-500/20 rounded-xl rounded-tl-sm px-3 py-2.5">
            <p className="text-xs text-rose-400">{error}</p>
            <button onClick={onRegenerate} className="mt-2 flex items-center gap-1 text-[10px] text-rose-400/80 hover:text-rose-400 transition-colors cursor-pointer">
          ...
          <div className="space-y-2">
            {result.explanation && (
              <div className="bg-monokai-surface border border-monokai-border rounded-xl rounded-tl-sm px-3 py-2">
                <p className="text-xs text-monokai-fg-muted leading-relaxed">{result.explanation}</p>
              </div>
            )}
            ...
            <div className="grid grid-cols-3 gap-1.5">
              <button
                onClick={handleCopy}
                className="flex items-center justify-center gap-1 px-2 py-1.5 text-[11px] rounded-lg bg-monokai-surface border border-monokai-border text-monokai-fg-muted hover:text-monokai-fg hover:border-monokai-border-strong transition-colors cursor-pointer"
              >
              ...
              <button
                ...
                className="flex items-center justify-center gap-1 px-2 py-1.5 text-[11px] rounded-lg bg-monokai-surface border border-monokai-border text-monokai-accent hover:bg-monokai-sidebar transition-colors cursor-pointer font-medium"
              >
...
const InputForm: React.FC<{
  ...
  <div className="space-y-2.5">
    ...
    <input
      ...
      placeholder="分析概念 *"
      className="w-full px-3 py-1.5 text-xs bg-monokai-bg border border-monokai-border rounded-lg text-monokai-fg placeholder-monokai-fg-muted/50 focus:outline-none focus:border-monokai-accent transition-colors"
    />
    <input
      type="text"
      value={property}
    ...
    <textarea
      ...
      placeholder="业务背景（可选）"
      rows={1}
      className="w-full px-3 py-1.5 text-xs bg-monokai-bg border border-monokai-border rounded-lg text-monokai-fg placeholder-monokai-fg-muted/50 focus:outline-none focus:border-monokai-accent transition-colors resize-none"
    />

    <button
      onClick={onGenerate}
      disabled={isGenerating || !concept.trim()}
      className="w-full flex items-center justify-center gap-1.5 px-3 py-2 text-xs font-semibold rounded-lg bg-monokai-accent text-monokai-bg hover:brightness-105 disabled:opacity-30 disabled:cursor-not-allowed shadow-xs transition-colors cursor-pointer"
    >
      {isGenerating ? (
  ...
  return (
    <div className="flex flex-col h-full bg-monokai-surface">
      ...
      <div className="flex items-center justify-between px-3 py-2.5 border-b border-monokai-border bg-monokai-bg">
        <div className="flex items-center gap-2">
          <div className="w-5 h-5 rounded-md bg-monokai-surface border border-monokai-border flex items-center justify-center text-monokai-accent">
            <Sparkles className="w-3 h-3" />
          </div>
        ...
        <div className="flex items-center gap-1">
          ...
          <button
            ...
            className="p-1 rounded-md hover:bg-monokai-bg text-monokai-fg-muted hover:text-monokai-accent transition-colors"
            title="新建会话"
          >
        ...
        {showSessionList && (
          <div className="w-44 border-r border-monokai-border flex flex-col bg-monokai-bg/40 overflow-y-auto">
            <div className="p-2">
              <button
                onClick={handleNewSession}
                className="w-full flex items-center justify-center gap-1.5 px-2 py-1.5 text-[11px] font-medium rounded-lg bg-monokai-surface border border-monokai-border text-monokai-fg hover:bg-monokai-sidebar transition-colors cursor-pointer shadow-xs"
              >
                <Plus className="w-3 h-3 text-monokai-accent" />
                新建会话
              </button>
            </div>
        ...
        <div className="flex-1 flex flex-col min-w-0">
          {!activeSession ? (
            ...
            <div className="flex-1 flex flex-col items-center justify-center gap-4 px-6">
              <div className="w-12 h-12 rounded-full bg-monokai-surface border border-monokai-border flex items-center justify-center text-monokai-accent shadow-xs">
              ...
              <button
                onClick={handleNewSession}
                className="flex items-center gap-2 px-4 py-2 text-xs rounded-lg bg-monokai-accent text-monokai-bg font-semibold hover:brightness-105 transition-colors shadow-xs cursor-pointer"
              >
                <Plus className="w-3.5 h-3.5" />
                创建第一个会话
              </button>
              ...
              >
                {messages.length === 0 && (
                  <div className="flex flex-col items-center justify-center h-full gap-3 text-center">
                    <div className="w-10 h-10 rounded-full bg-monokai-surface border border-monokai-border flex items-center justify-center text-monokai-accent">
                ...
                {aiError && activeSession?.messages[activeSession.messages.length - 1]?.role !== 'assistant' && (
                  <div className="px-3 py-2.5 bg-monokai-surface border border-rose-500/20 rounded-lg text-rose-400">
