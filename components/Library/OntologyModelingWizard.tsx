        ...
        return (
          <React.Fragment key={item.n}>
            ...
            <div className="flex flex-col items-center gap-1.5">
              <div className={`relative z-10 flex items-center justify-center w-7 h-7 rounded-full border text-xs font-semibold transition-colors
                ${done ? 'bg-monokai-surface text-monokai-green border-monokai-green/50' : active ? 'bg-monokai-surface text-monokai-yellow border-monokai-yellow' : 'bg-monokai-surface/40 text-monokai-comment border-monokai-border'}
              `}>
                {done
                  ? <Check className="w-3.5 h-3.5 text-monokai-green" strokeWidth={2.5} />
                  : <item.Icon className={`w-3.5 h-3.5 ${active ? 'text-monokai-yellow' : 'text-monokai-comment/60'}`} />
                }
              </div>
              <span className={`text-[10px] font-medium whitespace-nowrap transition-colors
                ${done ? 'text-monokai-green' : active ? 'text-monokai-yellow' : 'text-monokai-comment/60'}
              `}>
                {item.label}
              </span>
            </div>
            ...
            {i < 2 && (
              <div className="flex-1 mx-2 flex items-center min-w-[16px]">
                <div className={`h-px w-full rounded-full transition-colors
                  ${done ? 'bg-monokai-green/50' : 'bg-monokai-border'}
                `} />
              </div>
            )}
          </React.Fragment>
  ...
  return (
    <div
      className="group relative flex items-center gap-2 px-2.5 py-2 rounded-xl mb-1 last:mb-0
        ...
        hover:border-monokai-border-strong hover:bg-monokai-surface/25
  ...
  return (
    ...
    >
      ...
      <select
        ...
        className="w-24 h-6 px-2 text-xs bg-black/20 border border-monokai-amethyst/15 text-monokai-amethyst rounded-lg focus:outline-none focus:border-monokai-accent transition-colors appearance-none cursor-pointer"
      >
...
export const OntologyModelingWizard: React.FC<Props> = ({ onClose, onImport }) => {
  ...
  return (
    <ModalShell
      ...
      onClose={onClose}
      title="AI 本体建模助手"
      description="输入领域概念与业务逻辑，自动生成本体模型对象与关系"
      size="xl"
    >
      <div className="space-y-4">
        ...
        {step === 1 && !isLoading && (
          <div className="space-y-3.5">
            {/* Input card */}
            <div className="rounded-xl border border-monokai-border/80 bg-monokai-surface/40 p-4">
              <label className="block text-xs font-semibold text-monokai-amethyst mb-2 tracking-wide uppercase">
                领域概念描述
              </label>
              <textarea
                value={concept}
                ...
                rows={4}
                className="w-full h-24 px-3.5 py-2.5 text-xs bg-monokai-bg/60 border border-monokai-border text-monokai-fg placeholder-monokai-comment/40 rounded-lg focus:outline-none focus:border-monokai-accent transition-colors resize-none"
              />
              <p className="text-[11px] text-monokai-comment/60 mt-1.5 leading-relaxed">
              ...
              <div className="flex flex-wrap gap-1.5">
                {[
                  ...
                  <button
                    ...
                    className="text-[11px] px-2.5 py-1 rounded bg-monokai-surface/60 border border-monokai-border text-monokai-comment hover:text-monokai-fg hover:border-monokai-accent/40 transition-colors cursor-pointer"
                  >
                    {tpl.label}
                  </button>
                ))}
            ...
            {/* Error */}
            {error && (
              <div className="flex items-center gap-2 px-3.5 py-2.5 rounded-lg bg-monokai-orange/10 border border-monokai-orange/25 text-xs text-monokai-orange">
            ...
            {/* Pipeline hint */}
            <div className="flex items-start gap-2.5 px-3.5 py-3 rounded-lg bg-monokai-surface/30 border border-monokai-border/60">
              <Sparkles className="w-4 h-4 text-monokai-yellow shrink-0 mt-0.5" />
              <div className="space-y-1">
                <p className="text-xs text-monokai-fg font-medium leading-none">AI 流水线</p>
                <PipelineRow />
              </div>
            </div>
        ...
        {step === 2 && !isLoading && (
          <div className="space-y-3.5">
            ...
            <div>
              ...
              <div>
                {objectTypes.length === 0 ? (
                  <div className="py-4 text-center text-xs text-monokai-comment">暂无对象类型</div>
                ) : (
            ...
            <div>
              ...
              <div className="max-h-[220px] overflow-y-auto custom-scrollbar pr-0.5">
                {objects.length === 0 ? (
                  <div className="py-4 text-center text-xs text-monokai-comment">暂无实例对象</div>
                ) : (
                  objects.map((obj, i) => (
                    <div key={i}>
