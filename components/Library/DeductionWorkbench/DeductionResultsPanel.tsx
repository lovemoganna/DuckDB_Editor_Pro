...
import {
  AlertTriangle, BarChart3, Filter, Play, Search, Sparkles,
} from 'lucide-react';
import type {
  InferenceReport,
  SituationCandidate,
...
export interface DeductionResultsPanelProps {
...
export const DeductionResultsPanel: React.FC<DeductionResultsPanelProps> = ({
  report,
  objectNamesById,
  selectedFeatureIds,
  topKLimit,
  ...
  return (
    <div className="flex h-full flex-col overflow-hidden bg-[#12141e] border-r border-white/10 p-3 text-xs">
      ...
      <div className="mb-3 shrink-0">
        <div className="flex items-center justify-between">
          ...
          <div className="flex items-center gap-2">
            ...
            <button
              ...
              className="flex items-center gap-1 rounded-lg bg-monokai-cyan px-2.5 py-1 text-[11px] font-black text-black transition-all hover:brightness-110 disabled:opacity-40"
            >
              <Play className="h-3 w-3" /> 执行推演
            </button>
          </div>
        </div>
        ...
        {report && (
          <div className="mt-2.5 grid grid-cols-4 gap-1.5 text-center text-[10px]">
            <div className="rounded-lg bg-monokai-green/10 p-1.5 text-monokai-green">
              已成立<br /><strong>{report.establishedCandidates.length}</strong>
            </div>
            <div className="rounded-lg bg-monokai-cyan/10 p-1.5 text-monokai-cyan">
              可能出现<br /><strong>{report.possibleCandidates.length}</strong>
            </div>
            <div className="rounded-lg bg-monokai-pink/10 p-1.5 text-monokai-pink">
              已排除<br /><strong>{report.excludedCandidates.length}</strong>
            </div>
            <div className="rounded-lg bg-monokai-yellow/10 p-1.5 text-monokai-yellow">
        ...
        <div className="mt-2.5 flex items-center justify-between gap-2">
          ...
          <div className="relative w-32">
            ...
            <input
              ...
              className="w-full rounded-lg border border-white/10 bg-[#0c0d12] py-0.5 pl-7 pr-2 text-[10px] text-monokai-fg outline-none focus:border-monokai-accent transition-colors"
              aria-label="搜索推演情形"
            />
          </div>
        </div>
      ...
      <div className="flex-1 space-y-2 overflow-auto pr-1">
        {!report ? (
          <div className="flex h-48 flex-col items-center justify-center rounded-xl border border-dashed border-white/10 p-6 text-center text-monokai-comment">
            <BarChart3 className="mb-2 h-8 w-8 text-monokai-cyan/30" />
            <p>点击上方「执行推演」挖掘交叉情形</p>
          </div>
