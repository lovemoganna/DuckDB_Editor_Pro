...
export const SkillInvoker: React.FC<SkillInvokerProps> = ({
  ...
  return (
    <div className="h-full flex flex-col bg-[#1e1f1c] text-monokai-fg" onKeyDown={handleKeyDown}>
      ...
      <div className="flex items-center gap-2 px-4 py-2.5 border-b border-[#3e3d32] bg-[#1e1f1c]/80 backdrop-blur-sm shrink-0">
        ...
        <div className="flex items-center gap-1.5 px-2 py-1 rounded bg-monokai-purple/10 border border-monokai-purple/20">
          ...
          <span className="text-[9px] font-bold uppercase tracking-widest font-mono text-monokai-purple">Tactical Center</span>
        </div>

        <div className="w-px h-5 bg-[#3e3d32]" />

        <button
          onClick={handleAIFill}
          disabled={isExecuting}
          className="flex items-center gap-1.5 px-3 py-1.5 text-[10px] font-bold uppercase tracking-wider font-mono border transition-all duration-200 disabled:opacity-30 hover:scale-[1.02] active:scale-[0.98]"
        ...
        <button
          onClick={handleClear}
          disabled={isExecuting}
          className="flex items-center gap-1.5 px-3 py-1.5 text-[10px] font-bold uppercase tracking-wider font-mono border border-[#3e3d32] text-monokai-comment hover:border-monokai-pink hover:text-monokai-pink hover:bg-monokai-pink/5 transition-all duration-200 disabled:opacity-30"
          title="清空输入"
        >
        ...
        <button
          onClick={handleReset}
          disabled={isExecuting}
          className="flex items-center gap-1.5 px-2 py-1.5 text-[10px] font-bold uppercase tracking-wider font-mono border border-[#3e3d32] text-monokai-comment hover:border-monokai-fg hover:text-monokai-fg hover:bg-monokai-fg/5 transition-all duration-200 disabled:opacity-30"
          title="恢复默认值"
        >
        ...
        <div className="ml-auto flex items-center gap-1.5 text-[9px] font-mono text-monokai-comment/60">
          ...
          <span className="ml-1 text-[9px]">Quick Run</span>
        </div>
      </div>
      ...
      {skill.examples && skill.examples.length > 0 && (
        <div className="px-4 pt-3 space-y-2 shrink-0">
          <div className="flex items-center gap-2 text-[10px] uppercase tracking-widest font-mono text-monokai-comment">
            <Sparkles className="w-3 h-3" style={{ color: accent }} />
            <span>Presets</span>
            <span className="text-[9px] text-monokai-comment/50">({skill.examples.length})</span>
          </div>
          <div className="flex flex-wrap gap-2">
            {skill.examples.map(example => (
              <button
                ...
                className="inline-flex items-center gap-1.5 px-2.5 py-1.5 text-[10px] font-mono border transition-all duration-200 hover:scale-[1.02] active:scale-[0.98]"
                style={{
                  borderColor: appliedExampleName === example.name ? accent : '#3e3d32',
                  color: appliedExampleName === example.name ? accent : '#75715e',
                  background: appliedExampleName === example.name ? `${accent}12` : '#272822',
                }}
              >
                <span
        ...
        <div className="flex gap-2">
          <div className="relative flex-1">
            <textarea
              ...
              rows={2}
              className="w-full px-3 py-2 text-[11px] font-mono bg-[#272822] border border-[#49483e] text-monokai-fg placeholder-monokai-comment/60 resize-none focus:outline-none transition-colors rounded"
              style={{ borderColor: aiSuggestion ? accent : '#3e3d32' }}
            />
            {aiSuggestion && (
              <button
                onClick={() => setAiSuggestion('')}
                className="absolute right-2 top-2 p-1 rounded text-monokai-comment hover:text-monokai-fg hover:bg-[#3e3d32]/50 transition-colors"
              >
          ...
          <button
            ...
            className="flex items-center gap-1.5 px-4 py-2 text-[10px] font-bold uppercase tracking-wider font-mono border transition-all duration-200 disabled:opacity-30 hover:scale-[1.02] active:scale-[0.98] shrink-0"
            style={{ borderColor: accent, color: accent, background: `${accent}08` }}
          >
            {isGeneratingSuggestion ? (
              <><Loader2 className="w-3.5 h-3.5 animate-spin" /> <span>生成中...</span></>
            ) : (
              <><Sparkles className="w-3.5 h-3.5" /> <span>生成</span></>
            )}
          </button>
        </div>
      </div>

      {/* ── Scrollable Content Area (Input Fields + Live Preview) ── */}
      <div className="flex-1 overflow-y-auto custom-scrollbar">
        ...
        <div className="px-4 pt-3 pb-4 shrink-0">
          <div className="border border-[#3e3d32] rounded-lg bg-[#272822] overflow-hidden">
            {/* Preview header */}
            <div className="flex items-center justify-between px-3 py-2 border-b border-[#3e3d32] bg-[#1e1f1c]/50">
              <div className="flex items-center gap-2">
                <div className="w-6 h-6 rounded bg-monokai-orange/10 border border-monokai-orange/20 flex items-center justify-center">
                  <Code className="w-3.5 h-3.5" style={{ color: '#fd971f' }} />
                </div>
                <span className="text-[10px] font-bold uppercase tracking-widest font-mono text-monokai-fg">Simulation Layer</span>
                {isGeneratingPreview && (
                  <span className="flex items-center gap-1 text-[9px] font-mono uppercase animate-pulse" style={{ color: accent }}>
              ...
              <div className="flex items-center gap-1">
                <button
                  ...
                  className="p-1.5 rounded text-monokai-comment hover:text-monokai-fg hover:bg-[#3e3d32]/50 transition-colors"
                  title="手动刷新"
                >
                ...
                {showLivePreview ? (
                  ...
                  <button
                    ...
                    className="flex items-center gap-1 px-2 py-1 text-[9px] font-mono rounded border border-[#3e3d32] text-monokai-comment hover:border-monokai-fg hover:text-monokai-fg hover:bg-[#3e3d32]/50 transition-all"
                  >
              ...
              <div className="relative">
                {isGeneratingPreview ? (
                  <div className="p-4 space-y-2">
                    <div className="h-2.5 bg-[#3e3d32] w-3/4 animate-pulse rounded" />
                    <div className="h-2.5 bg-[#3e3d32] w-1/2 animate-pulse rounded" />
                    <div className="h-2.5 bg-[#3e3d32] w-5/6 animate-pulse rounded" />
                  </div>
                  ...
                  <div className="max-h-52 overflow-auto custom-scrollbar">
                    <CodeMirror
                      ...
                      className="sql-preview-codemirror"
                    />
                  </div>
                ) : (
                  <div className="p-4 text-[11px] font-mono text-[#75715e] italic">
                    {liveSqlPreview || '// Complete required fields for SQL preview'}
                  </div>
                )}
                {liveSqlPreview && !liveSqlPreview.startsWith('//') && !isGeneratingPreview && (
                  <button
                    onClick={() => handleCopy(liveSqlPreview, 'livePreview')}
                    className="absolute top-3 right-3 p-1.5 rounded border border-[#3e3d32] text-monokai-comment hover:text-monokai-fg hover:border-monokai-fg hover:bg-[#3e3d32]/50 transition-all"
                    title="复制 SQL"
                  >
      ...
      {/* ── Execute Action (Fixed Bottom) ── */}
      <div className="px-4 py-4 border-t border-[#3e3d32] shrink-0 bg-[#1e1f1c]/80 backdrop-blur-sm">
        <button
          onClick={handleExecute}
          disabled={isExecuting}
          className="w-full flex items-center justify-center gap-2.5 px-6 py-3.5 font-bold text-[12px] uppercase tracking-widest font-mono rounded-lg transition-all duration-200 hover:scale-[1.01] active:scale-[0.99] disabled:hover:scale-100"
          style={{
        ...
        >
          {isExecuting ? (
            ...
            <><Play className="w-4 h-4 fill-current" /> Execute Tactical SQL</>
          )}
        </button>
      </div>
    </div>
